...
namespace Migatsu
{
    public partial class ProductInfoForm : Form
    {
        ...
        private void MasterFileForm_Load(object sender, EventArgs e)
        {
            ...
            populateProductList();                        
            populateAutoCompleteSearchProduct();

            //hide ID of product
            dataGridViewProductList.Columns[0].Visible = false;
            unselectData();
        }
        ...
        public void populateProductList()
        {
            ...
            string strSelect = "SELECT [Product id] as ID, Product_Name as [Product Name], Description, [quantity total] as [Quantity]  FROM FINALVIEWWHOLEINVENTORY where IsArchived=0 ORDER BY Product_Name";
        ...
        public void populateAutoCompleteSearchProduct()
        {
            conn = new SqlConnection(gd.ConString);
            conn.Open();
            ...
            if (dr.HasRows == true)
            {
                while (dr.Read())
                {
                    productCollection.Add(dr[0].ToString());
                }
            }
            dr.Close();
            textBoxSearchProduct.AutoCompleteMode = AutoCompleteMode.SuggestAppend;
            textBoxSearchProduct.AutoCompleteSource = AutoCompleteSource.CustomSource;
            textBoxSearchProduct.AutoCompleteCustomSource = productCollection;
        }
        ...
        public void unselectData()
        {
            ...
            {
                dataGridViewProductList[0, i].Selected = false;
            }
        }

        private void buttonSearch_Click(object sender, EventArgs e)
        {
            string prodCol = "Product Name";
            int rowCount = dataGridViewProductList.RowCount;
            for (int i = 0; i < rowCount; i++)
            {
                if (dataGridViewProductList[prodCol, i].Value.ToString().ToLower() == textBoxSearchProduct.Text.Trim().ToLower())
                {
        ...
        {
            int row = Convert.ToInt32(dataGridViewProductList.CurrentCell.RowIndex);
            if (row != null)
            {
                EditProduct ep = new EditProduct(dataGridViewProductList[0, row].Value.ToString());
                ep.ShowDialog();
            }
            populateProductList();
            dataGridViewProductList[1, row].Selected = true;
